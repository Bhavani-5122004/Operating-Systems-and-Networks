...
for (int k = 0; k < arrind; k++) {
    ...
    r  = recvfrom(sock, ack, sizeof(ack), 0,(struct sockaddr*)&serverAddress, &addr_size);
     if (r == -1) {
     ...
     char seqnum[10];
    snprintf(seqnum, sizeof(seqnum), "%d", arr[k]->num);
   // printf("Ack: %s Seqnum: %s\n",ack,seqnum);
    if(strcmp(ack,seqnum)!=0 && arr[k]->num!=-1){
...
printf("\nTransmissions: \n");
 for(h =0;h<len;h++){
char seqnum2[10];
if(rand()%3==0 && receivedData.num!=-1){
  snprintf(seqnum2, sizeof(seqnum2), "%d",-1);
          flag = 1;
        count++;
}
else{
  elemcount++;
  if(receivedData.num!=-1){
printf("%d %s\n",receivedData.num,receivedData.info);
    ans[receivedData.num].num=receivedData.num;
    strcpy(ans[receivedData.num].info,receivedData.info);
    }
   snprintf(seqnum2, sizeof(seqnum2), "%d", receivedData.num);

}

  int s = sendto(sock, seqnum2, sizeof(seqnum2), 0, (struct sockaddr*)&serverAddress, sizeof(serverAddress));
   if (s == -1) {
...
memset(&receivedData1, 0, sizeof(receivedData1));
 ...
 printf("Total number of chunks transmitted successfully the first time: %d out of %d\n",elemcount-1,len-1);

 printf("Final transmitted string: \n");
 for(int jj=0;jj<len-1;jj++){
  printf("%s",ans[jj].info);
 }
 printf("\n");
